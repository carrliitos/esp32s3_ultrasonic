...
// Handler for POST requests at /sensor-data
static esp_err_t sensor_data_handler(httpd_req_t *req) {
  ...
  httpd_resp_sendstr(req, "{\"status\":\"success\",\"message\":\"Data received\"}");
  return ESP_OK;
}
...
esp_err_t send_data_to_web_server(float distance) {
  char post_data[50];
  snprintf(post_data, sizeof(post_data), "{\"distance\":%.2f}", distance);
  ...
  // Use credentials from Kconfig
  esp_http_client_set_username(client, BASIC_AUTH_USERNAME);
  esp_http_client_set_password(client, BASIC_AUTH_PASSWORD);

  esp_err_t err = esp_http_client_perform(client);
  if (err == ESP_OK) {
    ESP_LOGI(TAG, "POST request successful, Status = %d", esp_http_client_get_status_code(client));
  } else {
